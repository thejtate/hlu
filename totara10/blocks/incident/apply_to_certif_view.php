...
$certificationList = $incident->getFilteredCertificationList($userid);
...
echo html_writer::tag('div','<strong>Check all the certifications below to apply to this incident and click "Confirm".</strong> <br><br>');
echo "<div id='valError' style='color: red'></div>";
$mform->display();
...
$('#mform1').submit(function( event ) {
    var len = $('#mform1 input[type=checkbox]:checked').length;  
    if ( len == 0 ) {
        $('#valError').text('You must check at least one item from the list of licenses').show().fadeOut( 8000 );
            return false;
        }else{
            return true;
        }
